...
char* get_http_request(struct HTTP_HEADER_REQUEST* req){
	...
	if(req->parameters_size > 0){
			...
			param_size = strlen(req->parameters[0]);
		
			parameters = (char*)malloc(param_size*sizeof(char));
			
			//strcat(parameters,req->parameters[0]);
...
UARTWrite(1,buff);
	...
	if(param_size > 0){
		...
		if(strcmp(req->method, "GET") == 0){
			...
			str_req = (char*)malloc((strlen(req->method)+strlen(req->resource)+strlen(req->version)+strlen(req->content_type)+strlen(req->host)+strlen(req->authorization)+strlen(parameters)+tmp_param_size+2+34+2+14+18+6)*sizeof(char));
	...
	if(param_size == 0){ 
		...
		if(strcmp(req->method,"GET") == 0){
			...
			//sprintf(str_req,"%s %s %s\r\nUser-Agent: HTTPicus 1.0\r\nauthorization: N2U0ZGNmN2YtZDA1Ny00NzhhLTllZjctNDQ2NGEwN2U2ODU4Mzk5NDg5MTYtZTc4YS00OWZiLThlZjctMzVjNTY1ZTNkMDk0\r\nHost: %s\r\nContent-type: %s\r\nContent-Length: %d\r\n\r\n%s\r\n",req->method, req->resource,req->version,req->host,req->content_type,param_size,parameters);
			sprintf(str_req,"%s %s %s\r\nHost: %s\r\nUser-Agent: HTTPicus 1.0\r\nauthorization: %s\r\nContent-type: %s\r\nContent-Length: %d\r\n\r\n%s\r\n",req->method, req->resource,req->version,req->host,req->authorization,req->content_type,param_size,parameters);
...
char* create_chunked_post(struct HTTP_HEADER_REQUEST* req){
	...
	str_req = (char*)malloc((5+1+2+32+16+32+strlen(req->resource)+strlen(req->version)+strlen(req->host)+strlen(req->content_type)+strlen("authorization: N2U0ZGNmN2YtZDA1Ny00NzhhLTllZjctNDQ2NGEwN2U2ODU4Mzk5NDg5MTYtZTc4YS00OWZiLThlZjctMzVjNTY1ZTNkMDk0"))*sizeof(char));
